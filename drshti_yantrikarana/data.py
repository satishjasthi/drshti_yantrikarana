...
class TFRecords(object):
    ...
    def __init__(self, mode:str=None,
        ...
        if self.mode == 'train':
            logger.debug(f'Reading TrainHdf5_data from :{self.TrainHdf5_data.as_posix()}')
            logger.debug(f'Saving TrainTfRecord_data from :{self.TrainTfRecord_data.as_posix()}')
        if self.mode == 'test':
            logger.debug(f'Reading TestHdf5_data from :{self.TestHdf5_data.as_posix()}')
            logger.debug(f'Saving TestTfRecord_data from :{self.TestTfRecord_data.as_posix()}')


        logger.debug(f'Creating TF records for {self.mode} data')
        if self.mode == 'train':
            if self.TrainHdf5_data.exists():

                self.hdf5_data = tables.open_file(self.TrainHdf5_data.as_posix(), mode='r')
            else:
                logger.error('Error occured: '+ f'Unable to find {TrainHdf5_data.as_posix()}, '
    ...
    def writeTfRecord(self):
        ...
        logger.debug(f'Creating {self.mode} train records')
        if self.mode == 'train':
            ...
            logger.error('Error occured: '+ f'mode: {self.mode} is not valid, please choose mode between '
            f'(train, test)')
        ...
        logger.debug(f'Saving {self.mode} tf record files')
        if self.mode == 'train':
    ...
    def readTfRecord(self):
        ...
        :return:
        """
        logger.debug(f'Reading {self.mode} tf record files')
        if self.mode == 'train':
...
class ConvertData2Hdf5():
    ...
    def createHdf5File(self, directory):
        ...
        logger.debug(f'Created {hdf5_save_file}')
        ...
        if self.data_format == 'np_array':
            ...
            if self.save_augmentation_flag:
                logger.debug(f'Data save augmentation enabled, Creating dirs to store augmentations')
                directory_root = directory.parent
                logger.debug(f'directory_root: {directory_root.as_posix()}')
                save_aug_dir = directory_root / f'augmented_images'
                save_aug_dir.mkdir(parents=True, exist_ok=True)
                logger.debug(f'Creating save_aug_dir: {save_aug_dir.as_posix()}')
                train_aug_dir = save_aug_dir / f'train'
                train_aug_dir.mkdir(parents=True, exist_ok=True)
                logger.debug(f'Creating train_aug_dir: {train_aug_dir.as_posix()}')
                # create class wise dirs
                for classlabel in classes:
                    class_dir = train_aug_dir / f'{classlabel}'
                    class_dir.mkdir(parents=True, exist_ok=True)
                    logger.debug(f'Creating class_dir: {class_dir.as_posix()}')
                ...
                if not self.augment_bool:
                    ...
                    for aug_image, aug_label in zip(aug_images, aug_labels):
                        ...
                        if self.save_augmentation_flag:
                            ...
                            logger.debug(f'Saving augmented image: {dest.as_posix()}')
